...
QwenPaw 的[导入](./import) 可以将 Codex、Qoder 定时任务迁入当前智能体。
导入的任务类型为 **agent**，默认禁用、开启工具安全策略、投递到 Console，并保存结果
到收件箱。原应用的运行队列和执行历史不会恢复。

列表中的 **待迁移审核** 任务需要完成以下步骤：

1. 点击 **编辑**，核对提示词、执行时间、时区、投递设置和工作目录。
2. 如保留了远程或无法验证的来源目录标记，填写 **本地项目目录映射**，指向本机真实
   存在的项目目录，然后保存。其他迁入任务也可以按需修改此目录。
3. 点击 **审核通过**，再点击 **确认审核通过**。此操作完成审核，任务仍保持禁用。
4. 准备好后单独 **启用**，或在审核后 **立即执行** 一次验证结果。

审核前，启用和立即执行均不可用，普通编辑及 CLI 的 `resume` / `run` 也不能跳过审核。
无法转换、过期或缺少必要内容的定义需要修复后重试，不保证都会生成可运行任务。

绑定会话的 Codex heartbeat 依赖对应的来源聊天记录，并使用已导入的会话继续执行；
普通迁入任务使用独立任务会话。它不会修改 QwenPaw 的 [心跳](./heartbeat) 配置。
